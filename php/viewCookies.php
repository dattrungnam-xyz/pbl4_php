...
if (isset($_REQUEST['ID'])) {
  ...
  $db = new mysqli('localhost', 'root', '', 'pbl4_v2');
  if (mysqli_connect_errno()) exit;
  $sql = "Select * from cookies where IdBot = " . $id." ORDER BY Time DESC";
...
if (isset($_GET['ID'])) {
  $sql_pagination = 'SELECT COUNT(*) as NumRecord 
    FROM cookies where IdBot = ' . $_GET['ID'];
  ...
  if ($NumRecord > 15) {
    ...
    for ($i = 0; $i <
      ...
      if ($i != 0) {
        $href = 'viewCookies.php?ID=' . $_GET['ID'] . "&page=" . $i;
      } else {
        $href = 'viewCookies.php?ID=' . $_GET['ID'];
      }
      $numPage = $i + 1;
